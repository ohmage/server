...
public class CampaignServices {
	...
	public static void ensurePromptExistsInCampaign(Request request, String campaignId, String promptId) throws ServiceException {
		// Get the XML.
		String xml = CampaignDaos.getCampaignXml(campaignId);
		if(xml == null) {
			request.setFailed(ErrorCodes.CAMPAIGN_NOT_FOUND, "There is no such campaign with the campaign ID: " + campaignId);
		...
		if(nodes.size() == 0) {
			request.setFailed(ErrorCodes.CAMPAIGN_UNKNOWN_PROMPT_ID, "The following prompt ID is not part of the campaign's XML: " + promptId);
