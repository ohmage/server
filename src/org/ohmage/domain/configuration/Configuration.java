...
import org.ohmage.domain.configuration.Prompt.DisplayType;
import org.ohmage.domain.configuration.Prompt.Type;
import org.ohmage.domain.configuration.Prompt.ValueLabelPair;
import org.ohmage.domain.configuration.prompt.ChoicePrompt;
import org.ohmage.domain.configuration.prompt.CustomChoicePrompt;
import org.ohmage.domain.configuration.prompt.HoursBeforeNowPrompt;
import org.ohmage.domain.configuration.prompt.MultiChoiceCustomPrompt;
...
import org.ohmage.domain.configuration.prompt.SingleChoiceCustomPrompt;
...
/**
 * An immutable representation of a configuration. This includes all of the 
 * information that defines a configuration but none of the metadata associated
 * with one such as the user's associated with this configuration.
 * 
...
public class Configuration {
	...
	/**
	 * Creates a Configuration object with the given XML by parsing it and
	...
	public List<String> getSupervisors() {
		...
		for(String username : userRoles.keySet()) {
			if(userRoles.get(username).contains(Configuration.Role.SUPERVISOR)) {
	...
	public List<String> getAuthors() {
		...
		for(String username : userRoles.keySet()) {
			if(userRoles.get(username).contains(Configuration.Role.AUTHOR)) {
				usernames.add(username);
			}
		}
	...
	public List<String> getAnalysts() {
		...
		for(String username : userRoles.keySet()) {
			if(userRoles.get(username).contains(Configuration.Role.ANALYST)) {
				usernames.add(username);
			}
		}
	...
	public List<String> getParticipants() {
		...
		for(String username : userRoles.keySet()) {
			if(userRoles.get(username).contains(Configuration.Role.PARTICIPANT)) {
				usernames.add(username);
			}
		}
	...
	public RepeatableSet getRepeatableSet(String surveyId, String repeatableSetId) {
		...
		Map<String, SurveyItem> prompts = survey.getSurveyItems();
	 ...
	 * 									doesn't exist in that survey.
	 * 
	 * @see org.ohmage.domain.configuration.Prompt.Type
	 */
	...
	private String getChoiceValueFrom(Prompt prompt, String label) {
		Map<Integer, ValueLabelPair> choices;
		...
		for(ValueLabelPair valueLabelPair : choices.values()) {
	...
	private String getChoiceLabelFrom(Prompt prompt, String key) {
		...
		ValueLabelPair vlp = choices.get(keyInt);
		if(vlp == null) {
	 ...
	 */
	public Map<Integer, ValueLabelPair> getChoiceGlossaryFor(String surveyId, String promptId) {
		Prompt prompt = getPrompt(surveyId, promptId);
		if(prompt == null) {
			throw new IllegalArgumentException("No prompt with the given ID: " + promptId);
		}
	...
	private static Survey processSurvey(final Node survey) {
		...
		Map<String, SurveyItem> prompts = new HashMap<String, SurveyItem>(promptsList.size());
	...
	private static RepeatableSet processRepeatableSet(final Node repeatableSet,
		...
		Map<String, SurveyItem> promptMap = 
			new HashMap<String, SurveyItem>(prompts.size());
		for(SurveyItem currPrompt : prompts) {
			promptMap.put(currPrompt.getId(), currPrompt);
		}
	...
	private static Prompt processPrompt(final Node prompt, final int index) {
		...
		Map<String, ValueLabelPair> properties;
		...
		if(propertiesNodes.size() > 1) {
			...
			properties = new HashMap<String, ValueLabelPair>(0);
		}
	 ...
	 */
	private static Map<String, ValueLabelPair> getKeyValueLabelTrios(final Nodes properties) {
		if(properties.size() == 0) {
			return Collections.emptyMap();
		}
		
		Map<String, ValueLabelPair> result = new HashMap<String, ValueLabelPair>();
		...
		for(int i = 0; i < numProperties; i++) {
			...
			if(result.put(key, new ValueLabelPair(label, value)) != null) {
				throw new IllegalArgumentException(
	...
	private static HoursBeforeNowPrompt processHoursBeforeNow(final String id,
			...
			final Map<String, ValueLabelPair> properties, final int index) {
		
		int min;
		try {
			ValueLabelPair minVlp = 
	...
	private static MultiChoicePrompt processMultiChoice(final String id,
		...
		Map<Integer, ValueLabelPair> choices = 
			new HashMap<Integer, ValueLabelPair>(properties.size());
	...
	private static MultiChoiceCustomPrompt processMultiChoiceCustom(final String id,
		...
		return new MultiChoiceCustomPrompt(id, condition, unit, text, 
				...
				new HashMap<Integer, ValueLabelPair>(), defaultValues, index);
	}
	...
	private static PhotoPrompt processPhoto(final String id,
		...
		try {
			ValueLabelPair verticalResolutionVlp = 
	...
	private static RemoteActivityPrompt processRemoteActivity(final String id,
		...
		ValueLabelPair packageVlp = properties.get(RemoteActivityPrompt.KEY_PACKAGE);
		if(packageVlp == null) {
			throw new IllegalArgumentException(
					"Missing the '" + 
		...
		ValueLabelPair activityVlp = properties.get(RemoteActivityPrompt.KEY_ACTIVITY);
		if(activityVlp == null) {
			throw new IllegalArgumentException(
					"Missing the '" + 
		...
		ValueLabelPair actionVlp = properties.get(RemoteActivityPrompt.KEY_ACTION);
		if(actionVlp == null) {
			throw new IllegalArgumentException(
					"Missing the '" + 
		...
		ValueLabelPair autolaunchVlp = properties.get(RemoteActivityPrompt.KEY_AUTOLAUNCH);
		...
		int retries;
		try {
			ValueLabelPair retriesVlp = properties.get(RemoteActivityPrompt.KEY_RETRIES);
			if(retriesVlp == null) {
				throw new IllegalArgumentException(
						"Missing the '" + 
		...
		int minRuns;
		try {
			ValueLabelPair minRunsVlp = properties.get(RemoteActivityPrompt.KEY_MIN_RUNS);
			if(minRunsVlp == null) {
				throw new IllegalArgumentException(
						"Missing the '" + 
		...
		ValueLabelPair inputVlp = properties.get(RemoteActivityPrompt.KEY_INPUT);
		if(inputVlp == null) {
			throw new IllegalArgumentException(
					"Missing the '" + 
	...
	private static SingleChoiceCustomPrompt processSingleChoiceCustom(final String id,
		...
		return new SingleChoiceCustomPrompt(id, condition, unit, text, 
				abbreviatedText, explanationText, skippable, skipLabel, 
				displayType, displayLabel, choices, 
				new HashMap<Integer, ValueLabelPair>(), defaultValue, index);
	}
