...
import org.ohmage.validator.NumericValidators;
...
public abstract class VisualizationRequest extends UserRequest {
	...
	public VisualizationRequest(HttpServletRequest httpRequest) {
		...
		try {
			...
			if(tCampaignId == null) {
				setFailed(ErrorCodes.CAMPAIGN_INVALID_URN, "The campaign ID is missing.");
				throw new ValidationException("The campaign ID is missing.");
			}
			
			tWidth = NumericValidators.validateInteger(this, httpRequest.getParameter(InputKeys.VISUALIZATION_WIDTH), 0, MAX_IMAGE_DIMENSION);
			if(tWidth == null) {
			...
			tHeight = NumericValidators.validateInteger(this, httpRequest.getParameter(InputKeys.VISUALIZATION_HEIGHT), 0, MAX_IMAGE_DIMENSION);
			if(tHeight == null) {
