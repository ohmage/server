...
public class MobilityPrivacyStateCache extends StringAndIdCache {
	private static Logger _logger = Logger.getLogger(MobilityPrivacyStateCache.class);
	...
	private static synchronized void refreshMap() {
		...
		List<?> stateAndId;
		try {
			stateAndId = jdbcTemplate.query(SQL_GET_MOBILITY_PRIVACY_STATES_AND_IDS,
											new RowMapper() {
												@Override
			...
			_logger.error("Error executing SQL '" + SQL_GET_MOBILITY_PRIVACY_STATES_AND_IDS + "'. Aborting cache refresh.");
			return;
		}
